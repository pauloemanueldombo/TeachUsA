<?php
    ...
    if (isset($_GET["id"])){
        ...
        // Query
        $stmt = $con->prepare("SELECT `Sender`, `Message`, `Image` FROM Chat
        ...
        $getUser = $con->prepare("SELECT id, nome, foto_de_perfil FROM usuario WHERE (id LIKE ?) LIMIT 1");
        ...
        if ($count < 1) {
            ...
            while ($message = $result->fetch_assoc()) {
                if($message["Sender"] == $uid && $message["Image"] != "") {
                    ?>
                    <div class="row sent">
                        <img src="asset/uploads/<?php echo $message["Image"] ?>" />
                    </div>
                    <?php
            ...
            $stmt = $con->prepare("UPDATE conversas SET `lido` = 'n' WHERE (usuario_principal = ? AND outros_usuarios = ?)");
