<?php
    ...
    if(isset($_POST["message"]) && isset($_POST["id"])) {
        ...
        $checkConversation = $con->prepare("SELECT id_conversas FROM `conversas` WHERE (usuario_principal = ? AND outros_usuarios = ?)");
        ...
        if ($count < 1) {
            ...
            $createChat = $con->prepare("INSERT INTO `conversas` (`usuario_principal`, `outros_usuarios`, `lido`, `criado`) VALUES (?, ?, 'n', now())");
            $createChat->bind_param("ii", $uid, $user_id);
            $createChat->execute();

            // Create conversation other user side
            $createChat2 = $con->prepare("INSERT INTO `conversas` (`usuario_principal`, `outros_usuarios`, `lido`, `criado`) VALUES (?, ?, 'y', now())");
            $createChat2->bind_param("ii", $user_id, $uid);
            $createChat2->execute();
        } else {
            $update = $con->prepare("UPDATE `conversas` SET lido = 'y' WHERE (usuario_principal = ? AND outros_usuarios = ?)");
